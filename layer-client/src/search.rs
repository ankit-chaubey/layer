...
impl SearchBuilder {
    pub(crate) fn new(peer: tl::enums::Peer, query: String) -> Self {
        Self {
            peer,
            query,
    ...
    pub fn min_date(mut self, ts: i32) -> Self { self.min_date = ts; self }

    /// Only return messages on or before this Unix timestamp.
    pub fn max_date(mut self, ts: i32) -> Self { self.max_date = ts; self }

    /// Apply a `MessagesFilter` (e.g. photos only, video only, etc.).
    pub fn filter(mut self, f: tl::enums::MessagesFilter) -> Self { self.filter = f; self }

    /// Maximum number of messages to return (default 100).
    pub fn limit(mut self, n: i32) -> Self { self.limit = n; self }

    /// Start from this message ID (for pagination).
    pub fn offset_id(mut self, id: i32) -> Self { self.offset_id = id; self }

    /// Additional offset for fine-grained pagination.
    pub fn add_offset(mut self, off: i32) -> Self { self.add_offset = off; self }

    /// Only return messages with an ID ≤ `max_id`.
    pub fn max_id(mut self, id: i32) -> Self { self.max_id = id; self }

    /// Only return messages with an ID ≥ `min_id`.
    pub fn min_id(mut self, id: i32) -> Self { self.min_id = id; self }
    ...
    pub fn top_msg_id(mut self, id: i32) -> Self { self.top_msg_id = Some(id); self }
...
impl GlobalSearchBuilder {
    ...
    pub fn folder_id(mut self, id: i32) -> Self { self.folder_id = Some(id); self }

    /// Only return results from broadcast channels.
    pub fn broadcasts_only(mut self, v: bool) -> Self { self.broadcasts_only = v; self }

    /// Only return results from groups/supergroups.
    pub fn groups_only(mut self, v: bool) -> Self { self.groups_only = v; self }

    /// Only return results from private chats / bots.
    pub fn users_only(mut self, v: bool) -> Self { self.users_only = v; self }
    ...
    pub fn offset_rate(mut self, r: i32) -> Self { self.offset_rate = r; self }
