...
impl Photo {
    pub fn from_raw(raw: tl::types::Photo) -> Self { Self { raw } }
    ...
    pub fn access_hash(&self) -> i64  { self.raw.access_hash }
    pub fn date(&self)        -> i32  { self.raw.date }
    pub fn has_stickers(&self) -> bool { self.raw.has_stickers }
...
impl Document {
    pub fn from_raw(raw: tl::types::Document) -> Self { Self { raw } }
    ...
    pub fn access_hash(&self) -> i64    { self.raw.access_hash }
    pub fn date(&self)        -> i32    { self.raw.date }
    pub fn mime_type(&self)   -> &str   { &self.raw.mime_type }
    pub fn size(&self)        -> i64    { self.raw.size }
    ...
    pub fn is_animated(&self) -> bool {
        self.raw.attributes.iter().any(|a| matches!(a, tl::enums::DocumentAttribute::Animated))
    }
}
...
impl Sticker {
    ...
    pub fn from_document(doc: Document) -> Option<Self> {
        let has_sticker_attr = doc.raw.attributes.iter()
            .any(|a| matches!(a, tl::enums::DocumentAttribute::Sticker(_)));
        if has_sticker_attr { Some(Self { inner: doc }) } else { None }
    }
    ...
    pub fn is_video(&self) -> bool {
        self.inner.raw.attributes.iter()
...
impl Downloadable for Sticker {
    ...
    fn size(&self) -> Option<usize> { Some(self.inner.raw.size as usize) }
}
    ...
    pub async fn next(&mut self) -> Result<Option<Vec<u8>>, InvocationError> {
        ...
        match tl::enums::upload::File::deserialize(&mut cur)? {
            tl::enums::upload::File::File(f) => {
                ...
                if let Some(r) = &mut self.request { r.offset += req.limit as i64; }
                Ok(Some(f.bytes))
            }
...
impl Client {
    ...
    pub async fn upload_file(
        ...
        log::info!("[layer] uploaded '{}' ({} bytes, {} parts)", name, total, total_parts);
        Ok(UploadedFile { inner, mime_type: mime_type.to_string(), name: name.to_string() })
    }
    ...
    pub async fn upload_file_concurrent(
        ...
        while let Some(res) = tasks.join_next().await {
            res.map_err(|e| InvocationError::Io(
        ...
        log::info!("[layer] concurrent-uploaded '{}' ({} bytes, {} parts, {} workers)",
            name, total, total_parts, WORKER_COUNT);
        Ok(UploadedFile { inner, mime_type: mime_type.to_string(), name: name.to_string() })
    }
    ...
    pub async fn upload_stream<R: AsyncRead + Unpin>(
        ...
        if data.len() >= BIG_FILE_THRESHOLD {
            self.upload_file_concurrent(Arc::new(data), name, mime_type).await
        } else {
    ...
    pub async fn download_media_concurrent(
        ...
        while let Some((idx, data)) = rx.recv().await {
            if idx < parts.len() { parts[idx] = Some(data); }
        }
        ...
        for part in parts.into_iter().flatten() { out.extend_from_slice(&part); }
        out.truncate(size);
        Ok(out)
    }
    ...
    pub async fn download<D: Downloadable>(
        ...
        match item.size() {
            Some(sz) if sz >= BIG_FILE_THRESHOLD =>
                self.download_media_concurrent(loc, sz).await,
            _ =>
                self.download_media(loc).await,
        }
    }
}
